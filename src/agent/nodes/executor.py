    ...
    async def executor_node(state: AgentState) -> dict:
        ...
        for call in tool_calls:
            ...
            if server_name is None:
                content_str = json.dumps({"success": False, "error": f"Unknown tool: {tool_name}"})
                tool_result_messages.append({
                    "role": "tool", "name": tool_name, "tool_call_id": tool_call_id, "content": content_str,
                })
                continue

            target_employee_id = tool_args.get("employee_id", principal["user_id"])
            ...
            if not decision.allowed:
                result = {"success": False, "reason": decision.reason}
                ...
                if not approval.get("approved"):
                    result = {"success": False, "reason": "denied_by_human_approver"}
                else:
                    if server_name == "ops":
